	...
	void draw(DebugLine& dl, u16 window_width, u16 window_height)
	{
		...
		for (s32 cc = 0; cc < GRAPH_MAX_AXES; ++cc)
		{
			...
			for (s32 ii = 0, oldest = _head; ii < GRAPH_MAX_SAMPLES-1; ++ii)
			{
				...
				dl.add_line(
					  vector3(x_start + (ii+0)*x_step, value[0], 0.0f)
...
#define TEXT_PADDING 6.0f
		...
		char buf[64] = {};
		snprintf(buf, sizeof(buf), "%g", _range_min);
		draw_string(dl, buf, str_x, y_min + TEXT_PADDING);
		snprintf(buf, sizeof(buf), "%g", _range_max);
...
{
	void handle_command(ConsoleServer& cs, u32 client_id, JsonArray& args, void* /*user_data*/)
	{
...
namespace graph_globals
{
	...
	void draw_all(DebugLine& dl, u16 window_width, u16 window_height)
	{
		...
		{
			Graph* graph = (Graph*)container_of(cur, Graph, _node);
			graph->draw(dl, window_width, window_height);
		}
	}
